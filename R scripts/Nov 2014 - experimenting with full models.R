...
library(lme4)
lmer1<- glmer(botryllid ~ CO2.Treatment*Invasives*Week + (1|Mesocosm), family=poisson, data=repeatedm.stats)
summary(lmer1)
plot(lmer1)
qqnorm(residuals(lmer1));qqline(residuals(lmer1), col="red")


lmer2<- glmer(botryllid ~ CO2.Treatment*Invasives*Week + (1|Mesocosm/Tile), family=poisson, data=repeatedm.stats)
summary(lmer1)
plot(lmer1)
...
glmmadmb<-glmmadmb(formula = botryllid ~ CO2.Treatment*Invasives*Week + (1 | Mesocosm/Tile.ID), data = repeatedm.stats, family = "poisson",zeroInflation = FALSE) 
plot(glmmadmb)
summary(glmmadmb)
...
glmmadmb<-glmmadmb(formula = botryllid ~ CO2.Treatment*Invasives*Week + (1 | Mesocosm/Tile.ID), data = repeatedm.stats, family = "poisson",zeroInflation = TRUE) 
summary(glmmadmb)
...
beta<-fitdist((0.01*repeatedm.stats10$botryllid), "beta")
qqp(repeatedm.stats16$botryllid, "beta", shape1 = beta$estimate[[1]], shape2 = beta$estimate[[2]])

dx <- seq(0, 1, len = 200) 
plot(dx, dbeta(dx, shape1 = beta$estimate[[1]], shape2 = beta$estimate[[2]]), type = "l", col="red", ylim=c(0, 8)) 
d.AA <- density(repeatedm.stats16$botryllid)
lines(d.AA)
d.BB <- density(repeatedm.stats16$botryllid)
lines(d.BB)
...
z.PQL<- glmmPQL(botryllid ~ CO2.Treatment*Invasives, random=~1|Mesocosm/Tile.ID, family=poisson, data=repeatedm.stats12)
plot(z.PQL)
...
############ BINOMIAL
head(repeatedm.stats12)
z.binomial<- glmmPQL(botryllid ~ CO2.Treatment*Invasives, random=~1|Mesocosm/Tile.ID, family=binomial, data=repeatedm.stats12)
plot(z.binomial)
...
z5<-lmer(botryllid ~ CO2.Treatment*Invasives + (1|Mesocosm), REML=TRUE, data=repeatedm.stats)
plot(z5)
summary(z5)
